...
/-- The frozen Collins--Gisin coefficient polynomial on six deterministic bits. -/
def i3322Bits (a0 a1 a2 b0 b1 b2 : Bool) : ℤ :=
  - bit a0 - 2 * bit b0 - bit b1
    + bit a0 * bit b0
    + bit a0 * bit b1
    + bit a0 * bit b2
    + bit a1 * bit b0
...
def i3322CG (a b : DeterministicResponse) : ℤ :=
  i3322Bits (a 0) (a 1) (a 2) (b 0) (b 1) (b 2)
...
theorem pv_eq_cg_after_swaps (a b : DeterministicResponse) :
  ...
  simp [i3322PV, i3322CG, i3322Bits, swap12]
  ring

/-- Closed exhaustive computation of all `2^6 = 64` deterministic assignments. -/
theorem deterministic_bits_local_bound :
    ∀ a0 a1 a2 b0 b1 b2 : Bool,
      i3322Bits a0 a1 a2 b0 b1 b2 ≤ 0 := by
  decide

/-- The deterministic/local bound in the frozen response-function convention. -/
theorem deterministic_local_bound (a b : DeterministicResponse) :
    i3322CG a b ≤ 0 :=
  deterministic_bits_local_bound (a 0) (a 1) (a 2) (b 0) (b 1) (b 2)
